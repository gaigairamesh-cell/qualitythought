...
export default function AiInterviewRoom({
  ...
  const handleExitInterview = (confirmFirst: boolean = true) => {
    if (confirmFirst) {
      if (!window.confirm("Exit current interview room? Your session progress and active camera/audio streams will be shut down and lost.")) {
        return;
      }
    }
  ...
  const toggleVoiceRecording = () => {
    ...
    if (isRecordingVoice) {
      ...
      rec.continuous = false; // Auto-stop and submit when user stops speaking
      ...
      rec.onend = () => {
        setIsRecordingVoice(false);
        // Take the voice input and directly analyze the answer without entering manually
  ...
  const handleSendMessage = async (textOverride?: string) => {
    ...
    if (!textToSubmit.trim() || isLoadingMessage) return;
