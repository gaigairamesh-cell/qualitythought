        ...
        <div className="bg-slate-900 text-white p-3 border-b border-slate-800 flex items-center justify-between gap-3 shrink-0">
          <div className="flex items-center gap-3">
            <div className="relative w-12 h-12 shrink-0">
              {/* Animated speaking rings — expand outward while the AI is talking */}
              {isAiSpeaking && (
                <>
                  <span className="absolute inset-0 rounded-xl border-2 border-indigo-400/70 animate-ping" style={{ animationDuration: "1.4s" }} />
                  <span className="absolute -inset-1.5 rounded-xl border border-indigo-400/40 animate-ping" style={{ animationDuration: "1.8s", animationDelay: "0.2s" }} />
                </>
              )}
              <div className="relative w-12 h-12 bg-gradient-to-b from-slate-800 to-slate-950 rounded-xl border border-indigo-500/40 flex items-center justify-center overflow-hidden shadow-md">
                <div className={`absolute inset-0 bg-indigo-500/10 transition-opacity ${isLoadingMessage ? 'animate-pulse' : ''}`} />
                <svg
                  viewBox="0 0 100 100"
                  className={`w-9 h-9 relative z-10 transition-transform duration-300 ${isAiSpeaking ? "scale-105" : ""}`}
                  style={isLoadingMessage ? { animation: "aiHeadTilt 1.6s ease-in-out infinite" } : undefined}
                >
                  <defs>
                  ...
                  {/* Antenna */}
                  <line x1="50" y1="8" x2="50" y2="20" stroke="#a5b4fc" strokeWidth="3" strokeLinecap="round" />
                  <circle
                    cx="50" cy="6" r="5"
                    fill={isLoadingMessage ? "#fbbf24" : isRecordingVoice ? "#f43f5e" : isAiSpeaking ? "#818cf8" : "#34d399"}
                    style={{ animation: (isLoadingMessage || isAiSpeaking || isRecordingVoice) ? "aiAntennaPulse 0.9s ease-in-out infinite" : undefined }}
                  />
                  {/* Head */}
                  <rect x="18" y="20" width="64" height="58" rx="18" fill="url(#robotHeadGrad)" stroke="#4f46e5" strokeWidth="2.5" />
                  {/* Ears */}
                  <rect x="10" y="40" width="8" height="18" rx="4" fill="#6366f1" />
                  <rect x="82" y="40" width="8" height="18" rx="4" fill="#6366f1" />
                  {/* Eyes (blink via CSS keyframes on scaleY) */}
                  <g style={{ transformOrigin: "35px 45px", animation: "aiBlink 4s ease-in-out infinite" }}>
                    <circle cx="35" cy="45" r="7" fill="#1e1b4b" />
                    <circle cx="37" cy="42.5" r="2" fill="#e0e7ff" />
                  </g>
                  <g style={{ transformOrigin: "65px 45px", animation: "aiBlink 4s ease-in-out infinite" }}>
                    <circle cx="65" cy="45" r="7" fill="#1e1b4b" />
                    <circle cx="67" cy="42.5" r="2" fill="#e0e7ff" />
                  </g>
                  {/* Mouth: waveform bars while speaking, flat smile otherwise */}
                  {isAiSpeaking ? (
                    <g>
                      {[0, 1, 2, 3, 4].map((i) => (
                        <rect
                          ...
                          fill="#1e1b4b"
                          style={{
                            transformOrigin: `${32 + i * 9 + 2.5}px 66px`,
                            animation: `aiTalkBar 0.5s ease-in-out infinite`,
                    ...
                    <path d="M 34 64 Q 50 72 66 64" stroke="#1e1b4b" strokeWidth="3.5" strokeLinecap="round" fill="none" />
                  )}
                </svg>
                <span className={`absolute top-1 right-1 z-20 w-2.5 h-2.5 rounded-full border border-slate-900 ${
                  isLoadingMessage ? 'bg-amber-400 animate-pulse' : isRecordingVoice ? 'bg-rose-500 animate-ping' : isAiSpeaking ? 'bg-indigo-400 animate-pulse' : 'bg-emerald-400'
                }`} />
              </div>
              <style>{`
                @keyframes aiBlink {
            ...
            <div>
              <div className="flex items-center gap-1.5">
                ...
                <span className={`text-[8px] px-1.5 py-0.2 rounded-full font-bold uppercase ${
                  isLoadingMessage 
                    ...
                    : isAiSpeaking
                    ? "bg-indigo-500/10 text-indigo-300 border border-indigo-400/30 animate-pulse"
                    : "bg-emerald-400/10 text-emerald-400 border border-emerald-400/25"
                }`}>
                  {isLoadingMessage ? "💭 Thinking" : isRecordingVoice ? "🎤 Listening" : isAiSpeaking ? "🗣️ Speaking" : "🟢 Standby"}
                </span>
              </div>
              <p className="text-[9px] text-slate-400 leading-snug font-medium mt-0.5">
                {isLoadingMessage 
                  ...
                  : isAiSpeaking
                  ? "Reading the question aloud — listen or follow along in the chat."
                  : "Ready. Click the microphone below or start typing to answer."}
              </p>
            </div>
          </div>
          {(isRecordingVoice || isLoadingMessage || isAiSpeaking) && (
            <div className="flex gap-0.5 items-center justify-center h-6 px-3">
              {[1, 2, 3, 4, 5, 6].map((bar) => {
                ...
                return (
                  <span 
                    key={bar} 
                    style={{ animationDelay: delays[bar - 1], animationDuration: isRecordingVoice ? "0.6s" : isAiSpeaking ? "0.45s" : "1.2s" }}
                    className={`w-1 rounded-full animate-bounce ${isRecordingVoice ? 'bg-rose-500 h-5' : isAiSpeaking ? 'bg-indigo-400 h-5' : 'bg-indigo-400 h-3'}`}
                  />
                );
              })}
            </div>
