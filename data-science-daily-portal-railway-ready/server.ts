...
4. Final structural match similarity score (0 to 100).`;
  ...
  try {
    ...
    console.error("[Gemini API] Compare code error:", err);
    res.status(500).json({ error: "Failed to run automated comparison analysis: " + err.message });
  }
});
...
`;
  ...
  try {
    ...
    console.error("[Gemini API] Resume analysis error:", err);
...
`;
  ...
  try {
    ...
    console.error("[Gemini API] Build resume error:", err);
    res.status(500).json({ error: `Resume compilation failed: ${err.message}` });
  }
});
